...
for i=1:length(dcm_files)
    ...
    if (isempty(sliceix)==0)
        ...
        for j=1:length(sliceix)
            ...
            point1(1) = test_info(ix,5) + cos(test_info(ix,2))*thickness_est_pixel/2;
            point1(2) = test_info(ix,6) + sin(test_info(ix,2))*thickness_est_pixel/2;
            point2(1) = test_info(ix,5) - cos(test_info(ix,2))*thickness_est_pixel/2;
            point2(2) = test_info(ix,6) - sin(test_info(ix,2))*thickness_est_pixel/2;
