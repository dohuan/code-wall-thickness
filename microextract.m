function data = microextract(Cproject,Mproject,Cmaski,Mmaski,...
    Cbegslice,Cendslice,Mbegslice,L,x,spp,show,h_d)
...
for idx = 1:ceil((Cendslice-Cbegslice+1)/spp)
    ...
    for k = 1:x % lateral index
        ...
        for j = 1:x % slice index                
            slice = i+round(((j-(x+1)/2)*Cpsize)/Csliceinc);
            spaceslice(j) = (slice-i)*Csliceinc;

            if slice < 1 || slice > nC % slice is out of range
                line = nan(N,1);
            else % get image profileline data
                I = Cproject.images(slice).image;
